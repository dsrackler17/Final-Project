...
# Random Forest & VotingClassifier (sklearn)
...
def select_model():
    ...
    # Voting Ensemble only if we have at least two gradient boosters
        ...
        if can_ensemble and choice == "6":
            ...
            if "1" in models_available:
                estimators.append(('xgb', XGBClassifier(
                    n_estimators=300, max_depth=6, scale_pos_weight=4,
                    random_state=42, eval_metric='logloss')))
            if "3" in models_available:
                estimators.append(('lgb', LGBMClassifier(
                    n_estimators=300, max_depth=7, scale_pos_weight=4,
                    random_state=42, verbose=-1)))
...
def main():
    ...
    if model is None:
        print("No model available. Install at least XGBoost or use Random Forest.")
        return
    ...
    print("\n" + "="*80)
    print(f"TOP 20 NRXP CLONES — {len(top)} FOUND ({TODAY}) | Model: {model_name}")
    print("="*80)
